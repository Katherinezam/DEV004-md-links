...
const CLI = () => {
  ...
  if (helpOption) {
    console.log(chalk.bold.cyan(`Uso: node cli.js <ruta> [--validate] [--stats] [--help]`));
  ...
  // Mostrar mensaje de error si no se ha proporcionado la ruta del archivo o carpeta
    ...
    console.log(chalk.bold.red('Por favor, ingrese una ruta de archivo o carpeta.'));
  }
            ...
            .then((responses) => {
              ...
              // Mostrar el resultado de la validación de los enlaces
                ...
                links.forEach((link) => {
                  ...
                  console.log(`${link.file} ${link.href} ${result} ${link.status} ${link.message}`);
                });
              }
            })
            .catch((error) => {
              ...
              links.forEach((link) => {
                console.log(chalk.bold.red(`Ocurrió un error durante la validación de ${link.href}: ${error.message}`));
              });
            });
        }
        // Mostrar estadísticas de los enlaces si se ha proporcionado la opción --stats
